...
public class TestJcrArticleSearchService extends JcrTestBase {
    ...
    public void testFindArticles() throws StoreException, SearchException {
        ...
        updateArticle(site2, "4", "http://www.xyz.de/4", 2010, 1, 12, "Title", SCORE3);
        String path5 = updateArticle(site, "5", "http://www.abc.de/5", 2010, 1, 12, "Title", SCORE5);
        String path6 = updateArticle(site, "6", "http://www.abc.de/6", 2010, 1, 12, "Title", UNRELATED);
    
        List<String> sites = new ArrayList<String>();
        sites.add(site.getName());
        sites.add(site2.getName());
        List<EntityKey> result = articleSearchService.findArticles(sites, "Food Issue");
        Assert.assertEquals(1, result.size());
        Assert.assertTrue(result.contains(new EntityKey(null, path6)));
